...
function(input, output, session) {
    ...
    output$about <- renderUI({
        tags$div(
            ...
            HTML(paste("where δ",tags$sub("1")," represents the mean difference of gene expression between AA and AC, and δ",tags$sub("2")," 
                               represents the mean difference of gene expression between CC and AC, p is the minor allele frequency, q = 1-p, 
            ...
            paste0("This R shiny application is developed by Xiaoqi Li, under the supervisions of Drs. 
    ...
    observeEvent(input$radio,{
        ...
        if(input$radio=="One-way unbalanced ANOVA")
        {
            updateNumericInput(session, inputId = "delta1", label  = "Mean difference of gene expression (δ)", value = input$delta1)
            show("delta2")
            output$description2 <- renderUI({
                tags$div(
                    ...
                    HTML(paste0("The figure above shows the statistical power of an eQTL analysis as a function of the minor 
                                allele frequency (MAF) with different sample size. Unbalanced one-way ANOVA test was used to 
                                test the expression level across genotype classes, as Ref. (GTEx consortium, Nature Genetics, 
                                2013). We model the expression data as log-normally distributed with a log standard deviation 
                                of ", input$sigma1, " (we assume that each genotype class of subjects have the same standard 
                                ...
                                genotype classes (denoted by δ1 and δ2). In this case, δ=", input$delta1," is equivalent to 
                                detecting a log-expression change similar to the ",input$delta1/input$sigma1, "-fold of standard
                                deviation.")),
            ...
            output$description2 <- renderUI({
                tags$div(
                    ...
                    HTML(paste0("The figure above shows the statistical power of an eQTL analysis as a function of the minor
                                ...
                                1998). We model the expression data as log-normally distributed with a log standard deviation
                                of ", input$sigma1, ". This level of noise can be based on estimates from previous study or pilot data.
                                The effect size depends both on the MAF of the SNPs and the slope of linear regression (denoted
                                by β). In this case, β=", input$delta1, " is equivalent to detecting a ", input$delta1/input$sigma1,
                                "-fold of expression level chance across the genotype classes. ")),
    ...
    output$description <- renderUI({
        ...
        HTML(paste0("The figure above shows the statistical power of an eQTL analysis as a function of the minor allele
                    ...
                    to characterize the linear association between gene expression and genotype. We model the expression
                    data as log-normally distributed with a log standard deviation of ", input$sigma, " and an intra-class
                    correlation of ", input$rho, ". This level of noise can be based on estimates from previous study or 
                    pilot data. 
                    The effect size depends both on the MAF of the SNPs and the slope of linear regression (denoted by β).
                    In this case, β=", input$delta, " is equivalent to detecting a ", input$delta/input$sigma,"-fold of 
                    expression level chance across the genotype classes. ")),
    ...
    t_est <- reactive({
        if (input$radio_test == "One-way unbalanced ANOVA")
        {
            ...
            rownames(tab)<-c("<strong>Model used</strong>", "<strong>Power level</strong>", 
                             ...
                             "<strong>Mean differences of gene expression (δ;δ2)</strong>",
    ...
    output$Explanation1 <- renderUI({
        tags$div(
            ...
            tags$a( "‘powerEQTL’", href = "https://CRAN.R-project.org/package=powerEQTL"),
            paste0(". More details can be found in the online manual of the function 'powerEQTL.ANOVA' and 'powerEQTL.SLR' in R package "),
            tags$a( "‘powerEQTL’", href = "https://CRAN.R-project.org/package=powerEQTL"),
            tags$br(),
            tags$br(),
